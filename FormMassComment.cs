        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < textBox3.Lines.Length; i = i + 1)
            {
                ...
                foreach (WallGet.Item item in wg.response.items)
                {
                    ...
                    WebClient clieNT = new WebClient();
                    string AnswER = clieNT.DownloadString(RequeST);
                    RequestParam rp = JsonConvert.DeserializeObject<RequestParam>(AnswER);
                    Thread.Sleep(Number);
                    //Нужно сделать!!!!!! показ капчи пользователю
                    if(AnswER.Contains("Captcha needed"))
                    {
                        Error er = JsonConvert.DeserializeObject<Error>(AnswER);
                        pictureBox1.ImageLocation = er.captcha_img;
                        //подождать 
                        ...
                        string RequEST = "https://api.vk.com/method/wall.createComment?" + "owner_id=" + ug.response[0].id.ToString() + "&post_id=" + item.id + "&message=" + textBox2.Text + "&captcha_sid=" + er.captcha_sid + "&captha_key=" + textBox1.Text + "&" + access_token + "&v=5.124";
